...
const handleSearch = () => {
  ...
  searchTerm.timeout = setTimeout(async () => {
    if (searchTerm.query !== '') {
      const res = await fetch(`https://api.weatherapi.com/v1/search.json?key=acd852848e8a4cdebe6111630240503&q=${searchTerm.query}`)
...
const getWeather = async (id) => {
  const res = await fetch(`https://api.weatherapi.com/v1/forecast.json?key=acd852848e8a4cdebe6111630240503&q=id:${id}&days=3&aqi=no&alerts=no`)
  const data = await res.json();
    ...
    <form>
      <div class="bg-black border border-indigo-600/30 rounded-lg shadow-lg flex items-center">
        ...
        <input type="text" placeholder="Search for a ville"
          ...
          v-model="searchTerm.query" @input="handleSearch" />hey
      </div>
    </form>
    <!-- search suggestions -->
